...
import { TOLERANCE, curvePieces, positionKey as key } from './split.js';
...
export function buildProfile(geometry: readonly SketchGeometry[]): ProfileResult {
  ...
  for (const entity of geometry) {
    ...
    // Cut by whatever touches it. Without this the tracer, which turns corners only
    // where segments share an end, treated a curve as a wall: a chord across a circle
    // enclosed nothing and a line running into the middle of another left both open.
    const pieces = curvePieces(geometry, entity);
    if (pieces.length === 0) continue;

    if (entity.type === 'circle' && pieces.length === 1) {
      const segment: ProfileSegment = {
        kind: 'circle', centre: centreOf(pieces[0]!, entity.radius), radius: entity.radius,
      };
    ...
    for (const piece of pieces) {
      if (entity.type === 'line' || !piece.angles) {
        chainable.push({ kind: 'line', from: piece.from, to: piece.to });
        continue;
      }
      chainable.push({
        kind: 'arc', centre: centreOf(piece, entity.radius), radius: entity.radius,
        from: piece.from, to: piece.to,
        startAngle: piece.angles.start, endAngle: piece.angles.end,
      });
    }
  }
...
/** A piece's centre, recovered from its start point and the angle it starts at. */
function centreOf(piece: { from: Vec2; angles: { start: number } | null }, radius: number): Vec2 {
  const start = piece.angles!.start;
  return { x: piece.from.x - radius * Math.cos(start), y: piece.from.y - radius * Math.sin(start) };
}
